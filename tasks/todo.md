...
- **Follow-up (same day)**: live-preview polish — heading text flush with body text (hidden `# ` now includes the space) and hanging indents for wrapped list items and quotes (`listIndent.ts`, prefix width measured with canvas). Verified with a DevTools render check (15/15, pixel-exact alignment) and a screenshot. Version 0.2.1. Then YAML frontmatter: a lezer block-parser extension (`frontmatterParser.ts`, unit-tested) keeps it out of the markdown structure and `frontmatter.ts` styles the lines as a muted mono block; render check 19/19. Version 0.2.2.
